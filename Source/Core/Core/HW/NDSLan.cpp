...
LAN::LAN() noexcept : Inited(false)
{
  ...
  if (enet_initialize() != 0)
  {
    ...
    GENERIC_LOG_FMT(Common::Log::LogType::IOS_NET, Common::Log::LogLevel::LERROR, "LAN: failed to initialize enet");
    return;
  }

  GENERIC_LOG_FMT(Common::Log::LogType::IOS_NET, Common::Log::LogLevel::LINFO, "LAN: enet initialized");
  Inited = true;
}
...
LAN::~LAN() noexcept
{
  ...
  GENERIC_LOG_FMT(Common::Log::LogType::IOS_NET, Common::Log::LogLevel::LINFO, "LAN: enet deinitialized\n");
}
...
bool LAN::StartDiscovery()
{
  ...
  DiscoveryLastTick = (u32)Common::Timer::NowUs();
  DiscoveryList.clear();
...
bool LAN::StartClient(const char* playername, const char* host)
{
  ...
  ENetEvent event;
  int conn = 0;
  u32 starttick = (u32)Common::Timer::NowUs();
  const int conntimeout = 5000;
  while (true)
  {
    u32 curtick = (u32)Common::Timer::NowUs();
    ...
    if (enet_host_service(Host, &event, timeout) > 0)
    {
      ...
      else if (conn == 1 && event.type == ENET_EVENT_TYPE_RECEIVE)
      {
        ...
        cmd[1] = (u8)(kLANMagic & 0xFF);
        cmd[2] = (u8)((kLANMagic >> 8) & 0xFF);
        cmd[3] = (u8)((kLANMagic >> 16) & 0xFF);
        cmd[4] = (u8)((kLANMagic >> 24) & 0xFF);
        cmd[5] = (u8)(kProtocolVersion & 0xFF);
        cmd[6] = (u8)((kProtocolVersion >> 8) & 0xFF);
        cmd[7] = (u8)((kProtocolVersion >> 16) & 0xFF);
        cmd[8] = (u8)((kProtocolVersion >> 24) & 0xFF);
...
void LAN::ProcessDiscovery()
{
  ...
  u32 tick = (u32)Common::Timer::NowUs();
  ...
  if (IsHost)
  {
    ...
    sendto(DiscoverySocket, (const char*)&beacon, sizeof(beacon), 0, (const sockaddr_t*)&saddr,
           sizeof(saddr));
  }
  else
  {
    ...
    while (true)
    {
      ...
      int rlen =
          recvfrom(DiscoverySocket, (char*)&beacon, sizeof(beacon), 0, (sockaddr_t*)&raddr, &ralen);
      if (rlen < sizeof(beacon))
        continue;
      if (beacon.Magic != kDiscoveryMagic)
        continue;
      if (beacon.Version != kProtocolVersion)
        continue;
...
void LAN::ProcessHostEvent(ENetEvent& event)
{
  ...
  case ENET_EVENT_TYPE_RECEIVE:
  {
    ...
    {
      if (event.packet->dataLength != (9 + sizeof(Player)))
        break;
...
void LAN::ProcessLAN(int type)
{
  ...
  u32 time_last = (u32)Common::Timer::NowUs();
  ...
  time_last = (u32)Common::Timer::NowUs();
  ...
  while (enet_host_service(Host, &event, timeout) > 0)
  {
    if (event.type == ENET_EVENT_TYPE_RECEIVE && event.channelID == Chan_MP)
    {
      ...
      if (event.packet->dataLength < sizeof(MPPacketHeader))
        ...
        good = false;
      else if (header->SenderID == (u32)MyPlayer.ID)
        good = false;
      ...
      else
      {
        ...
        header->Magic = (u32)Common::Timer::NowUs();
    ...
    if (type == 2)
    {
      u32 time = (u32)Common::Timer::NowUs();
      if (time < time_last)
        return;
...
u16 LAN::RecvReplies(int inst, u8* packets, u64 timestamp, u16 aidmask)
{
  ...
  while (true)
  {
    ...
    if (good)
    {
      ...
      if (((myinstmask & ConnectedBitmask) == ConnectedBitmask) || ((ret & aidmask) == aidmask))
      {
