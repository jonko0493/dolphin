...
#define mpi() (NDS::MPInterface::Get())
...
#undef BBREG_FIXED
  ...
  RFChannelData[0][0] = 65536;
  RFChannelData[0][1] = 65536;
...
{
  bool on = Enabled && ((IOPORT(W_PowerUS) & 0x1) == 0);
...
void Wifi::SetIRQ14(int source)  // 0=USCOMPARE 1=BEACONCOUNT 2=forced
{
  ...
  if (source == 2)
    DEBUG_LOG_FMT(IOS_NET, "wifi: weird forced IRQ14");
...
void Wifi::UpdatePowerStatus(int power)  // 1=on 0=no change -1=off
{
  ...
  if (reqflags & 2)
  {
    ...
    {
      DEBUG_LOG_FMT(IOS_NET, "wifi: TRX power ON");
  ...
  else
  {
    ...
    if (curflags & 2)
      DEBUG_LOG_FMT(IOS_NET, "wifi: TRX power OFF");
...
void Wifi::TXSendFrame(const TXSlot* slot, int num)
{
  ...
  {
    if (num == 1)
      noseqno = (IOPORT(W_TXSlotCmd) & 0x4000) ? 1 : 0;
  }
  ...
  case 2:
  case 3:
    mpi().SendPacket(0, TXBuffer, 12 + len, USTimestamp);
    break;

  case 1:
    *(u16*)&TXBuffer[12 + 24 + 2] = MPClientMask;
    mpi().SendCmd(0, TXBuffer, 12 + len, USTimestamp);
    break;

  case 5:
    IncrementTXCount(slot);
    mpi().SendReply(0, TXBuffer, 12 + len, USTimestamp, IOPORT(W_AIDLow));
    break;
...
void Wifi::StartTX_LocN(int nslot, int loc)
{
  ...
  //  DEBUG_LOG_FMT(IOS_NET, "wifi: unusual loc %d bits set %04X", loc,
  //    IOPORT(W_TXSlotLoc1 + (loc * 4)));
...
void Wifi::StartTX_Cmd()
{
  ...
  //  WARN_LOG_FMT(IOS_NET, "wifi: !! unusual TXSLOT_CMD bits set %04X", IOPORT(W_TXSlotCmd));
...
void Wifi::SendMPDefaultReply()
{
  ...
  *(u32*)&reply[0xC + 0x18] = 0;
  ...
  /*int txlen = */mpi().SendReply(0, reply, 12 + 28, USTimestamp, IOPORT(W_AIDLow));
  //DEBUG_LOG_FMT(IOS_NET, "wifi: sent %d/40 bytes of MP default reply", txlen);
}
...
void Wifi::SendMPReply(u16 clienttime, u16 clientmask)
{
  ...
  // TODO: how does it behave if the packet addr is changed before it gets sent? (maybe just not
  // possible)
  if (slot->Valid)
  {
    slot->CurPhase = 0;
...
void Wifi::SendMPAck(u16 cmdcount, u16 clientfail)
{
  ...
  *(u32*)&ack[0xC + 0x1C] = 0;
  ...
  /*int txlen = */mpi().SendAck(0, ack, 12 + 32, USTimestamp);
  //DEBUG_LOG_FMT(IOS_NET, "wifi: sent %d/44 bytes of MP ack, %d %d\n", txlen, ComStatus, RXTime);
}
...
bool Wifi::ProcessTX(TXSlot* slot, int num)
{
  ...
  case 0:  // preamble done
  {
    ...
    if (num == 0 || num == 2 || num == 3)
    {
      ...
      if ((framectl & 0x00FF) == 0x0010)
      {
        ...
        //if (aid)
        //  DEBUG_LOG_FMT(IOS_NET, "[HOST] syncing client %04X, sync=%016llX", aid, USTimestamp);
      }
      else if ((framectl & 0x00FF) == 0x00C0)
      {
        if (IsMPClient)
        {
          INFO_LOG_FMT(IOS_NET, "[CLIENT] deauth");
          IsMP = false;
          IsMPClient = false;
        }
      }
  ...
  case 1:  // transmit done
  {
    ...
    if (num == 1)
    {
      ...
      u16 res = 0;
      if (MPClientMask)
        res = mpi().RecvReplies(0, MPClientReplies, USTimestamp, MPClientMask);
      MPClientFail &= ~res;
  ...
  case 2:  // MP host transfer done
  {
    ...
    if (slot->Rate == 2)
      slot->CurPhaseTime = 32 * 4;
    else
      slot->CurPhaseTime = 32 * 8;
  ...
  case 3:  // MP host ack transfer (reply wait done)
  {
    ...
    *(u16*)&RAM[slot->Addr + 0x2] = MPClientFail;
    ...
    else
    {
      ...
      IOPORT(W_TXSlotBeacon) = 0x8000;
      IOPORT(W_USCompareCnt) = 0x0003;
      IOPORT(W_BeaconCount1) = 0x0001;
...
void Wifi::FinishRX()
{
  ...
  case 2:  // data
  {
    ...
    // TODO: it seems to be impossible to receive a MP reply outside of a CMD transfer's reply
    // timeframe if the framectl subtype field is 1 or 5 maybe one of the unknown registers controls
    // that? maybe it is impossible to receive CF-Ack frames outside of a CF-Poll period?
  ...
  if ((rxflags & 0x800F) == 0x800C)
  {
    ...
    else
    {
      ...
      mpi().SendReply(0, nullptr, 0, USTimestamp, 0);
    }
  }
...
bool Wifi::CheckRX(int type)  // 0=regular 1=MP replies 2=MP host frames
{
  ...
  for (;;)
  {
    ...
    if (type == 0)
    {
      rxlen = mpi().RecvPacket(0, RXBuffer, &timestamp);
    }
    else
    {
      rxlen = mpi().RecvHostPacket(0, RXBuffer, &timestamp);
      if (rxlen < 0)
      {
        // host is gone
    ...
    {
      //ERROR_LOG_FMT(IOS_NET, "bad frame length %d/%d", framelen, rxlen - 12);
      continue;
    }
    ...
    {
      //DEBUG_LOG_FMT(IOS_NET, "received frame but bad channel %d (expected %d)", chan, CurChannel);
      continue;
    }
    ...
    if (type == 0 && (!IsMP))
    {
      if (MACEqual(&RXBuffer[12 + 16], MPReplyMAC) || MACEqual(&RXBuffer[12 + 4], MPCmdMAC) ||
  ...
  //DEBUG_LOG_FMT(IOS_NET, "wifi: received packet FC:%04X SN:%04X CL:%04X RXT:%d CMT:%d\n", framectl,
  //         *(u16*)&RXBuffer[12 + 4 + 6 + 6 + 6], *(u16*)&RXBuffer[12 + 4 + 6 + 6 + 6 + 2 + 2],
  //         framelen * 4, IOPORT(W_CmdReplyTime));
  ...
  *(u16*)&RXBuffer[8] = framelen;
  ...
  if ((frametype == 0x0010) && timestamp && macgood)
  {
    ...
    if (aid)
    {
      //DEBUG_LOG_FMT(IOS_NET, "[CLIENT %01X] host sync=%016llX\n", aid & 0xF, timestamp);
  ...
  else if (macgood && IsMPClient)
  {
    ...
    if (RXTimestamp < USTimestamp)
      RXTimestamp = USTimestamp;
    NextSync = RXTimestamp + (framelen * (txrate == 0x14 ? 4 : 8));
  ...
  if (ComStatus == 0)
  {
    ...
    if (txbusy)
    {
      ...
      else
      {
        ...
        if (txbusy & 0x0080)
          TXCurSlot = 5;
        else if (txbusy & 0x0010)
          TXCurSlot = 4;
        else if (txbusy & 0x0008)
          TXCurSlot = 3;
        else if (txbusy & 0x0004)
          TXCurSlot = 2;
        else if (txbusy & 0x0002)
          TXCurSlot = 1;
        else if (txbusy & 0x0001)
          TXCurSlot = 0;
      }
    }
    else
  ...
  if (ComStatus & 0x1)
  {
    ...
    if (!(RXTime & RXHalfwordTimeMask))
    {
      ...
      else if (addr == (IOPORT(W_RXBufReadCursor) << 1))
      {
        ...
        //    "wifi: RX buffer full (buf=%04X/%04X rd=%04X wr=%04X rxtx=%04X power=%04X com=%d "
        //    "rxcnt=%04X filter=%04X/%04X frame=%04X/%04X len=%d)\n",
        //    (IOPORT(W_RXBufBegin) >> 1) & 0xFFF, (IOPORT(W_RXBufEnd) >> 1) & 0xFFF,
        //    IOPORT(W_RXBufReadCursor), IOPORT(W_RXBufWriteCursor), IOPORT(W_RXTXAddr),
        //    IOPORT(W_PowerState), ComStatus, IOPORT(W_RXCnt), IOPORT(W_RXFilter),
        //    IOPORT(W_RXFilter2), *(u16*)&RXBuffer[0], *(u16*)&RXBuffer[12], *(u16*)&RXBuffer[8]);
        RXTime = 0;
        SetStatus(1);
...
void Wifi::ChangeChannel()
{
  ...
  //if (CurChannel > 0)
  //  WARN_LOG_FMT(IOS_NET, "wifi: switching to channel %d\n", CurChannel);
  //else
  //  WARN_LOG_FMT(IOS_NET, "wifi: invalid channel values %05X:%05X\n", val1, val2);
}
...
u16 Wifi::Read(u32 addr)
{
  ...
  case W_USCount0:
    return (u16)(USCounter & 0xFFFF);
  case W_USCount1:
    return (u16)((USCounter >> 16) & 0xFFFF);
  case W_USCount2:
    return (u16)((USCounter >> 32) & 0xFFFF);
  case W_USCount3:
    return (u16)(USCounter >> 48);

  case W_USCompare0:
    return (u16)(USCompare & 0xFFFF);
  case W_USCompare1:
    return (u16)((USCompare >> 16) & 0xFFFF);
  case W_USCompare2:
    return (u16)((USCompare >> 32) & 0xFFFF);
  case W_USCompare3:
    return (u16)(USCompare >> 48);

  case W_CmdCount:
    return (CmdCounter + 9) / 10;

  case W_BBRead:
    if ((IOPORT(W_BBCnt) & 0xF000) != 0x6000)
    {
      //ERROR_LOG_FMT(IOS_NET, "WIFI: bad BB read, CNT=%04X", IOPORT(W_BBCnt));
      return 0;
    }
  ...
  case W_RXBufDataRead:
    if (activeread)
    {
      ...
      rdaddr += 2;
      if (rdaddr == (u32)(IOPORT(W_RXBufEnd) & 0x1FFE))
        rdaddr = (u32)(IOPORT(W_RXBufBegin) & 0x1FFE);
...
void Wifi::Write(u32 addr, u16 val)
{
  ...
  case W_IFSet:
  {
    ...
    CheckIRQ(oldflags);
    //DEBUG_LOG_FMT(IOS_NET, "wifi: force-setting IF %04X", val);
  }
    return;
  ...
  case W_PowerDownCtrl:
    ...
    if (val != 0 && val != 3)
      //WARN_LOG_FMT(IOS_NET, "wifi: unusual W_PowerDownCtrl value %04X", val);
  ...
  case W_USCount0:
    USCounter = (USCounter & 0xFFFFFFFFFFFF0000) | (u64)val;
    return;
  case W_USCount1:
    USCounter = (USCounter & 0xFFFFFFFF0000FFFF) | ((u64)val << 16);
    return;
  case W_USCount2:
    USCounter = (USCounter & 0xFFFF0000FFFFFFFF) | ((u64)val << 32);
    return;
  case W_USCount3:
    USCounter = (USCounter & 0x0000FFFFFFFFFFFF) | ((u64)val << 48);
    return;

  case W_USCompare0:
    USCompare = (USCompare & 0xFFFFFFFFFFFF0000) | (u64)(val & 0xFC00);
    if (val & 0x0001)
      BlockBeaconIRQ14 = true;
    return;
  case W_USCompare1:
    USCompare = (USCompare & 0xFFFFFFFF0000FFFF) | ((u64)val << 16);
    return;
  case W_USCompare2:
    USCompare = (USCompare & 0xFFFF0000FFFFFFFF) | ((u64)val << 32);
    return;
  case W_USCompare3:
    USCompare = (USCompare & 0x0000FFFFFFFFFFFF) | ((u64)val << 48);
    return;
  ...
  case W_RXCnt:
    ...
    val &= 0xFF0E;
    //if (val & 0x7FFF)
      //WARN_LOG_FMT(IOS_NET, "wifi: unknown RXCNT bits set %04X", val);
    break;

  case W_RXBufDataRead:
    //WARN_LOG_FMT(IOS_NET, "wifi: writing to RXBUF_DATA_READ. wat");
    if (IOPORT(W_RXBufCount) > 0)
    {
      IOPORT(W_RXBufCount)--;
      if (IOPORT(W_RXBufCount) == 0)
  ...
  case W_TXSlotReset:
    if (val & 0x0001)
      IOPORT(W_TXSlotLoc1) &= 0x7FFF;
    if (val & 0x0002)
      IOPORT(W_TXSlotCmd) &= 0x7FFF;
    if (val & 0x0004)
      IOPORT(W_TXSlotLoc2) &= 0x7FFF;
    if (val & 0x0008)
      IOPORT(W_TXSlotLoc3) &= 0x7FFF;
    // checkme: any bits affecting the beacon slot?
    if (val & 0x0040)
      IOPORT(W_TXSlotReply2) &= 0x7FFF;
    if (val & 0x0080)
      IOPORT(W_TXSlotReply1) &= 0x7FFF;
    //if ((val & 0xFF30) && (val != 0xFFFF))
    //  WARN_LOG_FMT(IOS_NET, "unusual TXSLOTRESET %04X", val);
